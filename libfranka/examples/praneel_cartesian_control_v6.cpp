...
int main()
{
  ...
  double Kp = 2;         //This is the proportional control
  double time = 0;
  ...
  try{
    ...
    robot.control([&](const franka::RobotState& robot_state, franka::Duration period) -> franka::JointVelocities {
      ...
      if (time == 0.0) {
        ...
        //file_x << initial_pose[12] << std::endl;  //Writing to a file
      ...
      if (time > T[2] && time <= T[3]){
        ...
        desired_position[2] = initial_pose[14];
			...
			if (time > T[7] && time <= T[8]){
			  ...
			  V7 = V7 + A6*(time - T[7]) + 0.5*J2_peak*pow((time - T[7]),2);
