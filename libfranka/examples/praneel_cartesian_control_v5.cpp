  ...
  try{
    ...
    robot.control([&](const franka::RobotState& robot_state, franka::Duration period) -> franka::JointVelocities {
      ...
      if (time == 0.0) {
        ...
        file << initial_pose[12] << std::endl;  //Writing to a file
        file_time << time << std::endl;         //Writing to a file
      }
      if (time<=15){
        desired_position[0] = initial_pose[12] - 0.00029*pow(time,3) + 0.0064*pow(time,2) - 0.0055*time + 0.00023;
        desired_position[1] = initial_pose[13];
        desired_position[2] = initial_pose[14];
      }

      //std::cout << "Desired_position" << desired_position << std::endl;
      ...
      //franka::RobotState robot_state = robot.readOnce();
        ...
        if (time>15){
          desired_position[0] = 0.573;
          desired_position[1] = current_state[13];
          desired_position[2] = current_state[14];
        }
