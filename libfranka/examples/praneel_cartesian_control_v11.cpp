...
int main()
{
  ...
  double Kp = 2.5;         //This is the proportional control freaks out for 4  an 0.8 is like maximum
	double Kd = 1.5;

	double Kp_z = 1.6;         //This is the proportional control freaks out for 4  an 0.8 is like maximum
	double Kd_z = 0.03;
  ...
  try{
    ...
    errordot_x.open("errordotx.txt");
		...
		std::ofstream jointq2;
		jointq2.open("joint2.txt");
    ...
    robot.control([&](const franka::RobotState& robot_state, franka::Duration period) -> franka::JointVelocities {
      ...
      //franka::RobotState robot_state = robot.readOnce();
        ...
        //std::array<double,7>  joint_velc = robot_state.dq;
				...
				joint_angles = robot_state.q;
        std::array<double, 6> inbuilt_twist = robot_state.O_dP_EE_c;
        ...
        current_velocity << current_twist[0],current_twist[1],current_twist[2];
				...
				v_required = current_velocity.norm();

				current_velocity << v_required,0,0;
        ...
        file_time << time << std::endl;
				...
				jointq2 << joint_angles[1] << std::endl;

        std::array<double,6> x_dot;
        x_dot[0] = Kp*error[0] + Kd*error_dot[0];
        x_dot[1] = 0;
        x_dot[2] = Kp_z*error[2] + Kd_z*error_dot[2];  //Z-axis tracker
